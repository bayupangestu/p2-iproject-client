<template>
  <Navbar></Navbar>
  <h2 class="text-center text-2xl">{{ dataThread.title }}</h2>
  <div class="flex">
    <div class="grid grid-cols-12 gap-1.5 w-full p-2">
      <div class="p-2 md:col-start-2 md:col-span-2 border-grey-800 border-2 dark:bg-black">
        <p class="text-slate-300">{{ dataThread.user.username }}</p>
      </div>
      <div class="p-2 md:col-span-7 md:col-start-4 border-slate-300 border-2">
        <p class="text-slate-300">{{ dataThread.content }}</p>
      </div>
      <div class="fixed p-2 md:col-span-2">
        <button type="button" @click.prevent="addPostPage"
          class="text-slate-300 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
          Add Post
        </button>
      </div>
    </div>
  ...
  <div class="flex" v-for="post in dataThread.post" :key="post.id">
    <div class="grid grid-cols-12 gap-1.5 w-full p-2">
      ...
      <div class="p-2 md:col-span-7 md:col-start-4 border-slate-300 border-2">
        <p class="text-slate-300">{{ post.body }}</p>
      </div>
    </div>
  </div>
</template>
